...
const Player = () => {
	...
	return (
		<div
			...
			style={
				currentTrack
					? { height: "80px", opacity: "1", transition: "all 0.5s ease" }
					: {
							visibility: "hidden",
							height: "0px",
							opacity: "0",
			...
			<div className={styles.trackInfoBlock}>
				{currentTrack ? (
					<TrackInfo
						...
						imgPath={currentTrack?.imagePath}
					/>
				) : null}
			</div>
			<div className={styles.navigationBlock}>
				<div>
					<TrackNavigation
						...
						startTrack={handlePlay}
						stopTrack={handlePause}
					/>
				</div>
				<div>
					<TrackProgress
						allTime={allTimeFormat}
						currentTime={currentTimeFormat}
						minValue={currentTime}
						maxValue={durationTrack}
						onChnage={handleChangeTime}
					/>
				</div>
			</div>
