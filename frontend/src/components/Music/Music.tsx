...
const Music = ({
	...
	return (
		<div
			...
			style={isPlaying ? { borderColor: "#023e7d" } : { borderColor: "#202020" }}
		>
			<div className={styles.left}>
				...
				<div className={styles.info}>
					<div className={styles.title}>{musicData.title}</div>
					<div className={styles.author}>{musicData.author}</div>
				</div>
			</div>
			<div
				className={styles.right}
				style={
					!isLocal ? { gridTemplateColumns: "40px 40px" } : { gridTemplateColumns: "40px 40px 40px" }
				}
			>
				{isLocal == true ? (
				...
				{addFavorties || deleteFavorites ? (
					<button onClick={handleFavorties} className={styles.btn}>
						{favorties ? (
							<img src="/img/music/inFavorites.png" style={{ width: "15px" }} />
						) : (
							<img src="/img/music/notFavorites.svg" style={{ width: "15px" }} />
						)}
					</button>
				) : null}
				...
				<div ref={focusElementRef} onBlur={handleClose} tabIndex={0} className={styles.funcContainer}>
					{openFunc === true ? (
						<MusicFunc trackId={id} deleteTrack={deleteMusic} id={musicData._id} />
					) : null}
				</div>
			</div>
		</div>
